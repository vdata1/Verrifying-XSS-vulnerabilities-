<?php
  ...
  if($_POST["username"] != "" && $_POST["fname"] != "" && $_POST["lname"] != "")
  {
   ...
   while( $userlist = mysql_fetch_row($query) )
   {
	if($_POST["username"] == $userlist[0])
	{
	 ...
	 <input type='hidden' name='page' value=\"".htmlspecialchars($_POST[$page])."\">
	 </form>");
	}
   }

   // If all is good, insert the new teacher into the database //
   $query = mysql_query("INSERT INTO teachers VALUES('', \"".htmlspecialchars($_POST[username])."\", \"".htmlspecialchars($_POST[fname])."\", \"".htmlspecialchars($_POST[lname])."\")")
 ...
 {
  $query = mysql_query("UPDATE `teachers` SET `userid`=\"".htmlspecialchars($_POST[username])."\", `fname`=\"".htmlspecialchars($_POST[fname])."\", `lname`=\"".htmlspecialchars($_POST[lname])."\" WHERE `teacherid`=\"".htmlspecialchars($_POST[teacherid])."\" LIMIT 1")
