...
$selectclass = $_POST[selectclass];
 ...
 // Get the coursename //
 $query = mysql_query("SELECT coursename FROM courses WHERE courseid = \"".htmlspecialchars($_POST["selectclass"])."\" ") or die("ManageAssignments.php: Unable to get the course name - ".mysql_error());
 ...
 <form name='assignments' action='./index.php' method='POST'>
  ...
  <table cellspacing='0' width='600' cellpadding='8' class='dynamiclist'>
   <tr class='header'>
	<th colspan='6'><h2>\"".htmlspecialchars($coursename)."\"</th>
   </tr>
   <tr class='header'>
   ...
   $query = mysql_query("SELECT assignmentid, title, totalpoints, assigneddate, duedate, assignmentinformation FROM assignments WHERE courseid = \"".htmlspecialchars($selectclass)."\" ORDER BY assigneddate DESC")
...
print("\n</center>
  ...
  <input type='hidden' name='page2' value=\"".htmlspecialchars($_POST[page2])."\">
  <input type='hidden' name='onpage' value='\"".htmlspecialchars($_POST[onpage])."\">
  <input type='hidden' name='logout'>
  <input type='hidden' name='selectclass' value=\"".htmlspecialchars($_POST[selectclass])."\"/>
